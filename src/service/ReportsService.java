...
public class ReportsService {
    ...
    public void checkReports() {
        if (yearlyReportManager.yearlyTransactions.isEmpty() && monthlyReportManager.monthlyReports.isEmpty()) {
            System.out.println(InputHandler.ANSI_RED
                    + "Отчеты пока не сформированы. Воспользуйтесь для начала командой [1] и [2]\n"
            ...
            for (YearlyTransactions yearlyTransaction : yearlyReportManager.yearlyTransactions) {
                if (yearlyTransaction.isExpense) {
                    if (monthlyExpenses.get(yearlyTransaction.month) == yearlyTransaction.amount)
                        ...
                        printMismatch(yearlyTransaction.month, monthlyExpenses.get(yearlyTransaction.month),
                                yearlyTransaction.amount, true);
                    }
                } else {
                    if (monthlyIncome.get(yearlyTransaction.month) == yearlyTransaction.amount)
                        ...
                        printMismatch(yearlyTransaction.month, monthlyExpenses.get(yearlyTransaction.month),
                                yearlyTransaction.amount, false);
                    }
                }
            }
            System.out.println();
    ...
    private void printMismatch(String month, double monthlyAmount, double yearlyAmount, boolean isExpense) {
        if (isExpense) {
            System.out.println(InputHandler.ANSI_RED + "[!] Обнаружено несоответствие расходов за " + month
                    + InputHandler.ANSI_RED);
            System.out.println("В месячном отчете расходы составляют - " + monthlyAmount + ", в годовом - "
                    + yearlyAmount);
        } else {
            System.out.println(InputHandler.ANSI_RED + "[!] Обнаружено несоответствие доходов за " + month
                    + InputHandler.ANSI_RED);
            System.out.println("В месячном отчете доходы составляют - " + monthlyAmount + ", в годовом - "
                    + yearlyAmount);
        }
    }
    ...
    private void printMatch(String month, boolean isExpense) {
        if (isExpense)
            System.out.println(InputHandler.ANSI_WHITE + "[✔] Сверка расходов за " + month + " прошла успешно"
                    + InputHandler.ANSI_WHITE);
        else
            System.out.println(InputHandler.ANSI_WHITE + "[✔] Сверка доходов за " + month + " прошла успешно"
                    + InputHandler.ANSI_WHITE);
    }
}
